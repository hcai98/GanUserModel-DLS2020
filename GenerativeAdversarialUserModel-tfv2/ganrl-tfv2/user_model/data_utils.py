...
class Dataset(object):
    ...
    def __init__(self, args):
        ...
        # prepare feature data for each user
        print("------------- prepare data feature for each user... --------------")
        for user in tqdm(range(self.size_user)):
            ...
            click_t = 0
            for event in range(num_events): # iterate over events (displays) for user i
                disp_list = data_behavior[user][1][event] # get the list of item they see
                pick_id = data_behavior[user][2][event] # get the picked id
                for id in disp_list: # we log every new item seen in this event into a dictionary: itemid -> the n-th item seen
                    if id not in news_dict:
                        news_dict[id] = len(news_dict)  # for each user, news id start from 0
                id = pick_id
                self.data_click[user].append([click_t, news_dict[id]]) # so we log the picked event in the format (event#, item news #)
                self.feature_click[user][click_t] = item_feature[id] # it event t, we pick the item
                for idd in disp_list: # iterate over all displayed item
                    self.data_disp[user].append([click_t, news_dict[idd]]) # append the item in format (event#, n-th item)
                click_t += 1  # splitter a event with 2 clickings to 2 events

            self.data_news_cnt[user] = len(news_dict) # how many new item we see across all events
            self.feature[user] = np.zeros([self.data_news_cnt[user], self.f_dim]) # initialize item feature table

            for id in news_dict: # iterate over all items in newsdict => store their feature vector into featuture matrix
                self.feature[user][news_dict[id]] = item_feature[id]

            self.feature[user] = self.feature[user].tolist() # [[000100],[10000]...[01000]] every row an unique item
            self.feature_click[user] = self.feature_click[user].tolist() # every row => [event#, feature of item picked]
    ...
    def data_process_for_placeholder(self, user_set):
        ...
        if self.model_type == 'PW':
            ...
            size_user = len(user_set) # number of users
            max_time = 0 

            click_sub_index = []

            for u in user_set: # maximum # of events that any user experience
            ...
            for u_idx in range(size_user):
                ...
                for x in self.data_click[u]:
                    t, click_id = x # t = event when click, click-id is the news #
                    click_feature[t][u_idx] = self.feature[u][click_id] # retrieve item feature from news # => at time t the user pick this feature
                    u_t_clickid_tmp.append([u_idx, t, click_id]) # at time t, user "u_idx" picks the "clickid"-th item he saw
                    user_time_dense[u_idx, t] = 1.0 # mark at time t, u_idx has picked something
                ...
                click_sub_index_tmp = map(lambda x: u_t_dispid_tmp.index(x), u_t_clickid_tmp) # where in the dispid that this click happened

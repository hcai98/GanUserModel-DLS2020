...
if __name__ == '__main__':
    ...
    # call place holder function in model class
    ...
    val_log = []
    import os
    outname = 'train_log.csv'
    outname_val = 'val_log.csv'
    ...
    for i in range(cmd_args.num_itrs):
        ...
        if cmd_args.user_model == 'LSTM':
            ...
            t_opt, t_loss,t_prec1, t_prec2 = sess.run([train_opt, train_loss, train_prec1, train_prec2], feed_dict={user_model.placeholder['clicked_feature']: click_feature_tr,
            ...
            t_opt, t_loss,t_prec1, t_prec2 = sess.run([train_opt, train_loss, train_prec1, train_prec2], feed_dict={user_model.placeholder['disp_current_feature']: feature_tr,
        ...
        train_log.append([t_loss,t_prec1, t_prec2]) 
        pd.DataFrame(train_log, columns=['loss', 'prec1', 'prec2']).to_csv(fullname)
        ...
        if np.mod(i, 10) == 0:
            ...
            val_log.append([vali_loss_prc[0], vali_loss_prc[1], vali_loss_prc[2]]) 
            pd.DataFrame(val_log, columns=['loss', 'prec1', 'prec2']).to_csv(fullname_val)

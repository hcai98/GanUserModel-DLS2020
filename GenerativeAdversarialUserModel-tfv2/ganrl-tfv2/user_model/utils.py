...
def mlp(x, hidden_dims, output_dim, activation, sd, act_last=False):
    ...
    for h in hidden_dims:
        x = tf.compat.v1.layers.dense(x, h, activation=activation, trainable=True,
                            kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=sd))
        # x = tf.compat.v1.layers.BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True,trainable=True)(x)
    if act_last:
        return tf.compat.v1.layers.dense(x, output_dim, activation=activation, trainable=True,
                               kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=sd))
    else:
        return tf.compat.v1.layers.dense(x, output_dim, trainable=True,
                               kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=sd))
...
class UserModelLSTM(object):
    ...
    def construct_placeholder(self):
        ...
        self.placeholder['clicked_feature'] = tf.compat.v1.placeholder(tf.float32, (None, None, self.f_dim))  # (time, user=batch, f_dim)
        self.placeholder['ut_dispid_feature'] = tf.compat.v1.placeholder(tf.float32, shape=[None, self.f_dim])  # # (user*time*dispid, _f_dim)
    ...
    def construct_computation_graph(self):
        ...
        batch_size = tf.shape(input=self.placeholder['clicked_feature'])[1] # ckucjed feature: t => user id => feature of clicked item
        ...
        # construct lstm
        cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.rnn_hidden, state_is_tuple=True)
        initial_state = cell.zero_state(batch_size, tf.float32)
        rnn_outputs, rnn_states = tf.compat.v1.nn.dynamic_rnn(cell, self.placeholder['clicked_feature'], initial_state=initial_state, time_major=True)
        ...
        u_bar_feature = tf.transpose(a=u_bar_feature, perm=[1, 0, 2])  # (user, time, rnn_hidden)
        ...
        # utility: multilayer perceptrons
        ...
        disp_sum_exp_u_tensor = tf.sparse.reduce_sum(disp_exp_u_tensor, axis=2)
        sum_click_u_tensor = tf.sparse.reduce_sum(click_u_tensor, axis=2)

        loss_tmp = - sum_click_u_tensor + tf.math.log(disp_sum_exp_u_tensor + 1)  # (user, time) loss
        loss_sum = tf.reduce_sum(input_tensor=tf.multiply(self.placeholder['ut_dense'], loss_tmp))
        event_cnt = tf.reduce_sum(input_tensor=self.placeholder['ut_dense'])
        loss = loss_sum / event_cnt
        
        
        
        
        dense_exp_disp_util = tf.sparse.to_dense(disp_exp_u_tensor, default_value=0.0, validate_indices=False)

        click_tensor = tf.compat.v1.sparse_to_dense(self.placeholder['ut_clickid'], denseshape, self.placeholder['ut_clickid_val'], default_value=0.0, validate_indices=False)
        argmax_click = tf.argmax(input=click_tensor, axis=2)
        argmax_disp = tf.argmax(input=dense_exp_disp_util, axis=2)
        ...
        precision_1_sum = tf.reduce_sum(input_tensor=tf.multiply(self.placeholder['ut_dense'], argmax_compare))
        ...
        top2_compare = tf.reduce_sum(input_tensor=tf.cast(tf.equal(tf.reshape(argmax_click, tmpshape), tf.cast(top_2_disp, tf.int64)), tf.float32), axis=2)
        precision_2_sum = tf.reduce_sum(input_tensor=tf.multiply(self.placeholder['ut_dense'], top2_compare))
    ...
    def construct_computation_graph_u(self):
        ...
        batch_size = tf.shape(input=self.placeholder['clicked_feature'])[1]
        ...
        self.min_trainable_variables = tf.compat.v1.trainable_variables()
    ...
    def construct_computation_graph_policy(self):
        ...
        with tf.compat.v1.variable_scope('lstm2'):
            cell2 = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.rnn_hidden, state_is_tuple=True)
            initial_state2 = cell2.zero_state(batch_size, tf.float32)
            rnn_outputs2, rnn_states2 = tf.compat.v1.nn.dynamic_rnn(cell2, self.placeholder['clicked_feature'], initial_state=initial_state2, time_major=True)

        u_bar_feature2 = tf.concat([tf.zeros([1, batch_size, self.rnn_hidden], dtype=tf.float32), rnn_outputs2], 0)
        u_bar_feature2 = tf.transpose(a=u_bar_feature2, perm=[1, 0, 2])  # (user, time, rnn_hidden)
        ...
        disp_pi_dense_tensor = tf.sparse.add(a=(-10000.0) * tf.ones(tf.cast(denseshape, tf.int32)), b=disp_pi_tensor)
        ...
        pi_net = tf.compat.v1.math.softmax(disp_pi_dense_tensor)

        pi_net_val = tf.gather_nd(pi_net, self.placeholder['ut_dispid'])

        loss_max_sum = tf.reduce_sum(input_tensor=tf.multiply(pi_net_val, self.u_net - 0.5 * pi_net_val))
        event_cnt = tf.reduce_sum(input_tensor=self.placeholder['ut_dense'])

        loss_max = loss_max_sum / event_cnt

        sum_click_u_tensor = tf.reduce_sum(input_tensor=tf.gather(self.u_net, self.placeholder['click_sublist_index']))
        loss_min_sum = loss_max_sum - sum_click_u_tensor
        loss_min = loss_min_sum / event_cnt

        click_tensor = tf.compat.v1.sparse_to_dense(self.placeholder['ut_clickid'], denseshape, self.placeholder['ut_clickid_val'], default_value=0.0)
        argmax_click = tf.argmax(input=click_tensor, axis=2)
        argmax_disp = tf.argmax(input=pi_net, axis=2)
        ...
        max_trainable_variables = list(set(tf.compat.v1.trainable_variables()) - set(self.min_trainable_variables))
        ...
        self.init_variables = list(set(tf.compat.v1.global_variables()) - set(self.min_trainable_variables))
    ...
    def construct_model(self, is_training, reuse=False):
        ...
        if is_training:
            ...
            learning_rate = tf.compat.v1.train.exponential_decay(self.lr, global_step, 100000, 0.96, staircase=True)
...
class UserModelPW(object):
    ...
    def construct_computation_graph(self):
        ...
        for ii in range(self.pw_dim):
            position_weight = tf.compat.v1.get_variable('p_w'+str(ii), [self.band_size], initializer=tf.compat.v1.constant_initializer(0.0001))
            ...
            click_history[ii] = tf.sparse.sparse_dense_matmul(cumsum_tril_matrix, self.placeholder['Xs_clicked'])  # Xs_clicked: section by _f_dim
        ...
        sum_exp_disp_ubar_ut = tf.math.segment_sum(exp_u_disp, self.placeholder['disp_2d_split_sec_ind'])
        ...
        click_cnt = tf.sparse.reduce_sum(click_tensor, axis=1)
        loss_sum = tf.reduce_sum(input_tensor=- sum_click_u_bar_ut + tf.math.log(sum_exp_disp_ubar_ut + 1))
        event_cnt = tf.reduce_sum(input_tensor=click_cnt)
        loss = loss_sum / event_cnt

        exp_disp_ubar_ut = tf.SparseTensor(self.placeholder['disp_indices'], tf.reshape(exp_u_disp, [-1]), denseshape)
        dense_exp_disp_util = tf.sparse.to_dense(exp_disp_ubar_ut, default_value=0.0, validate_indices=False)
        argmax_click = tf.argmax(input=tf.sparse.to_dense(click_tensor, default_value=0.0), axis=1)
        argmax_disp = tf.argmax(input=dense_exp_disp_util, axis=1)

        top_2_disp = tf.nn.top_k(dense_exp_disp_util, k=2, sorted=False)[1]

        precision_1_sum = tf.reduce_sum(input_tensor=tf.cast(tf.equal(argmax_click, argmax_disp), tf.float32))
        precision_1 = precision_1_sum / event_cnt
        precision_2_sum = tf.reduce_sum(input_tensor=tf.cast(tf.equal(tf.reshape(argmax_click, [-1, 1]), tf.cast(top_2_disp, tf.int64)), tf.float32))
        precision_2 = precision_2_sum / event_cnt

        self.lossL2 = tf.add_n([tf.nn.l2_loss(v) for v in tf.compat.v1.trainable_variables() if 'bias' not in v.name]) * 0.05  # regularity
        return loss, precision_1, precision_2, loss_sum, precision_1_sum, precision_2_sum, event_cnt

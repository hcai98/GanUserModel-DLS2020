...
for user in tqdm(range(size_user)):
    data_behavior[user] = [[], [], []]
...
# this is especially for out project
if (cmd_args.dataset == 'rocketretail'):
    feat_table = pd.read_csv('./'+cmd_args.dataset+'_feature_table.txt')
    new_features = feat_table.to_numpy()
